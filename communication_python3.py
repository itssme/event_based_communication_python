...
class Connection(Thread):
    ...
    def run(self):
        while self.thread_running:
            try:
                msg = self.socket.recv(1024).decode()  # TODO: handle exception: connection reset by peer -> stop connection
...
class ConnectionClient(Connection):
    ...
    def connect(self, ip, port=MAIN_PORT):
        ...
        self.peer_number = json.loads(self.socket.recv(1024).decode())["peer"]
    ...
    def __gather_connections(self):
        while self.__is_gathering_connections:
            try:
                ...
                connection_socket.send(json.dumps({"peer": self.peers}).encode())
                self.peers += 1
